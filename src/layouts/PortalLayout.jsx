...
import {
  SquaresFour, UsersThree, BookOpenText, CalendarCheck, FileText, ArrowsLeftRight, ChartLineUp,
  FlagBanner, MegaphoneSimple, CalendarDots, ChalkboardTeacher, Robot, GearSix, MagnifyingGlass,
  List, X, CaretDown, SignOut, House, Bell, SidebarSimple,
...
const navItems = [
  { to: '/portal/dashboard', label: 'Dashboard', icon: SquaresFour, roles: ['Admin', 'Adviser', 'Subject Teacher'] },
  { to: '/portal/students', label: 'Student Records', icon: UsersThree, roles: ['Admin', 'Adviser'] },
  { to: '/portal/performance', label: 'Academic Performance', icon: BookOpenText, roles: ['Admin', 'Adviser', 'Subject Teacher'] },
  ...
  { to: '/portal/transfers', label: 'Transfer Records', icon: ArrowsLeftRight, roles: ['Admin'] },
  { to: '/portal/analytics', label: 'Analytics', icon: ChartLineUp, roles: ['Admin', 'Adviser', 'Subject Teacher'] },
  { to: '/portal/flagging', label: 'Student Flagging', icon: FlagBanner, roles: ['Admin', 'Adviser'] },
  { to: '/portal/announcements', label: 'Announcements', icon: MegaphoneSimple, roles: ['Admin', 'Adviser'] },
  { to: '/portal/events', label: 'Event Calendar', icon: CalendarDots, roles: ['Admin', 'Adviser'] },
  { to: '/portal/teachers', label: 'Teacher Management', icon: ChalkboardTeacher, roles: ['Admin'] },
  { to: '/portal/ai-assistant', label: 'AI Agent', icon: Robot, roles: ['Admin', 'Adviser', 'Subject Teacher'] },
  { to: '/portal/settings', label: 'Settings & Profile', icon: GearSix, roles: ['Admin', 'Adviser', 'Subject Teacher'] },
]
    ...
    <aside className={`${mobile ? `fixed inset-y-0 left-0 z-[70] transition-transform lg:hidden ${open ? 'translate-x-0' : '-translate-x-full'}` : `fixed inset-y-0 left-0 z-50 hidden transition-all lg:flex ${collapsed ? 'w-20' : 'w-64'}`} flex-col overflow-hidden bg-gradient-to-b from-[#061a31] via-navy-950 to-[#092c50] text-white shadow-2xl`}>
      ...
      <nav className="no-scrollbar relative flex-1 space-y-1 overflow-y-auto px-3 py-5">
        {allowed.map(({ to, label, icon: Icon }) => (
          ...
          >
            <Icon size={20} weight="duotone" className="shrink-0" />{(!collapsed || mobile) && <span>{label}</span>}
          </NavLink>
        ))}
      </nav>
      {!mobile && (
        <button onClick={() => setCollapsed(!collapsed)} className="m-3 flex items-center justify-center gap-2 rounded-xl border border-white/10 px-3 py-2.5 text-xs text-slate-300 hover:bg-white/10">
          <SidebarSimple size={18} weight="duotone" className={collapsed ? '' : 'rotate-180'} />{!collapsed && <span>Collapse menu</span>}
        </button>
      )}
    </aside>
  )
    ...
    <div className="portal-shell min-h-screen">
      ...
      <div className={`transition-all ${collapsed ? 'lg:pl-20' : 'lg:pl-64'}`}>
        <header className="sticky top-0 z-40 flex h-20 items-center justify-between border-b border-white/80 bg-white/80 px-4 shadow-[0_8px_30px_rgba(15,55,95,.04)] backdrop-blur-xl sm:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setMobileOpen(true)} className="rounded-xl p-2 text-navy-900 hover:bg-slate-100 lg:hidden" aria-label="Open navigation"><List size={23} weight="bold" /></button>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button className="relative rounded-xl p-2.5 text-slate-500 hover:bg-slate-100" aria-label="Notifications"><Bell size={20} weight="duotone" /><span className="absolute right-2 top-2 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white" /></button>
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl">
                  ...
                  <button onClick={() => navigate('/portal/settings')} className="mt-1 flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"><GearSix size={17} weight="duotone" />Profile settings</button>
                  <button onClick={() => navigate('/')} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-slate-700 hover:bg-slate-50"><House size={17} weight="duotone" />Public website</button>
                  <button onClick={() => { logout(); navigate('/login') }} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-rose-600 hover:bg-rose-50"><SignOut size={17} weight="duotone" />Log out</button>
                </div>
              )}
            </div>
          </div>
